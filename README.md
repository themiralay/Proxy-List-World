
# Free HTTP Proxy List 🌍

It is a lightweight project that, every 10 minutes, scrapes lots of free-proxy sites, validates if it works, and serves a clean proxy list.


> Scraper found **8859** proxies at the latest update. Usable proxies are below.

## Usage

Click the file format that you want and copy the URL.


|File|Content|Count|
|----|-------|-----|
|[data.txt](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data.txt)|`ip_address:port` combined (seperated new line)|1407|
|[data.json](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data.json)|`ip, port`|1407|
|[data-with-geolocation.json](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data-with-geolocation.json)|`ip, port, geolocation`|1407|

## Sources

|Source|Found Proxies|Succeed|
|------|-------------|-------|
|[free-proxy-list.net](https://free-proxy-list.net)|300|✅|
|[us-proxy.org](https://www.us-proxy.org)|200|✅|
|[proxydb.net](http://proxydb.net)|0|🚫|
|[free-proxy-list.com](https://free-proxy-list.com/?page=&port=&type%5B%5D=http&type%5B%5D=https&up_time=0&search=Search)|0|🚫|
|[proxy-list.download](https://www.proxy-list.download/HTTP)|0|🚫|
|[vpnoverview.com](https://vpnoverview.com/privacy/anonymous-browsing/free-proxy-servers)|0|🚫|
|[proxyscan.io](https://www.proxyscan.io)|0|🚫|
|[proxylist.geonode.com](https://proxylist.geonode.com/api/proxy-list?limit=300&page=1&sort_by=lastChecked&sort_type=desc&protocols=http,https)|300|✅|
|[proxyscrape.com](https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all)|3306|✅|
|[github.com/clarketm/proxy-list](https://raw.githubusercontent.com/clarketm/proxy-list/master/proxy-list-raw.txt)|400|✅|
|[github.com/monosans/proxy-list](https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/http.txt)|584|✅|
|[github.com/TheSpeedX/PROXY-List](https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt)|3769|✅|


## Sample Proxies With Geolocation Info

|#|Ip|Port|Country|City|Internet Service Provider|
|-|--|----|-------|----|-------------------------|
|1|157.120.38.97|3128|Japan|Minato|NTT PC Communications, Inc.|
|2|152.32.132.190|7890|Hong Kong|Hong Kong|UCLOUD INFORMATION TECHNOLOGY (HK) LIMITED|
|3|183.76.137.110|8080|Japan|Osaka|ASAHI Net, Inc.|
|4|45.125.67.37|8443|Hong Kong|Fanling|Tele Asia Limited|
|5|129.226.81.110|7890|Singapore|Singapore|Tencent Cloud Computing (Beijing) Co|
|6|210.223.44.230|3128|South Korea|Seoul|Korea Telecom|
|7|113.160.132.26|8080|Vietnam|Bắc Ninh|VietNam Post and Telecom Corporation|
|8|145.241.117.33|8888|United Arab Emirates|Dubai|Oracle Corporation|
|9|212.224.88.212|443|Germany|Neu-Isenburg|Ultahost, Inc.|
|10|147.45.178.211|14658|Germany|Frankfurt am Main|Global Connectivity Solutions LLP|
|11|87.120.205.164|444|Germany|Frankfurt am Main|WAIcore Ltd|
|12|185.221.237.57|8443|Germany|Frankfurt am Main|BitCommand LLC|
|13|84.47.150.125|1080|Russia|Moscow|LLC "Nauka-Svyaz"|
|14|113.192.31.79|8099|Indonesia|South Jakarta|PT Indo Telemedia Solusi|
|15|45.13.116.168|21537|France|Mathay|ABJP|
|16|62.133.60.126|24558|Germany|Frankfurt am Main|Global Connectivity Solutions LLP|
|17|15.204.238.117|3128|United States|Reston|OVH SAS|
|18|94.241.169.176|1080|Russia|St Petersburg|JSC "TIMEWEB"|
|19|77.110.119.136|3128|United States|Charlotte|Aeza International LTD|
|20|45.153.231.229|8080|Russia|Moscow|UFO Hosting LLC|



## Contributing

Contributions are welcome, and they are greatly appreciated! Every
little bit helps, and credit will always be given.

