
# Free HTTP Proxy List 🌍

It is a lightweight project that, every 10 minutes, scrapes lots of free-proxy sites, validates if it works, and serves a clean proxy list.


> Scraper found **9602** proxies at the latest update. Usable proxies are below.

## Usage

Click the file format that you want and copy the URL.


|File|Content|Count|
|----|-------|-----|
|[data.txt](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data.txt)|`ip_address:port` combined (seperated new line)|1527|
|[data.json](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data.json)|`ip, port`|1527|
|[data-with-geolocation.json](https://raw.githubusercontent.com/themiralay/Proxy-List-World/master/data-with-geolocation.json)|`ip, port, geolocation`|1500|

## Sources

|Source|Found Proxies|Succeed|
|------|-------------|-------|
|[free-proxy-list.net](https://free-proxy-list.net)|300|✅|
|[us-proxy.org](https://www.us-proxy.org)|200|✅|
|[proxydb.net](http://proxydb.net)|0|🚫|
|[free-proxy-list.com](https://free-proxy-list.com/?page=&port=&type%5B%5D=http&type%5B%5D=https&up_time=0&search=Search)|0|🚫|
|[proxy-list.download](https://www.proxy-list.download/HTTP)|0|🚫|
|[vpnoverview.com](https://vpnoverview.com/privacy/anonymous-browsing/free-proxy-servers)|0|🚫|
|[proxyscan.io](https://www.proxyscan.io)|0|🚫|
|[proxylist.geonode.com](https://proxylist.geonode.com/api/proxy-list?limit=300&page=1&sort_by=lastChecked&sort_type=desc&protocols=http,https)|300|✅|
|[proxyscrape.com](https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all)|4126|✅|
|[github.com/clarketm/proxy-list](https://raw.githubusercontent.com/clarketm/proxy-list/master/proxy-list-raw.txt)|400|✅|
|[github.com/monosans/proxy-list](https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/http.txt)|507|✅|
|[github.com/TheSpeedX/PROXY-List](https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt)|3769|✅|


## Sample Proxies With Geolocation Info

|#|Ip|Port|Country|City|Internet Service Provider|
|-|--|----|-------|----|-------------------------|
|1|168.110.52.228|3128|Japan|Tokyo|Oracle Corporation|
|2|152.32.132.190|7890|Hong Kong|Hong Kong|UCLOUD INFORMATION TECHNOLOGY (HK) LIMITED|
|3|183.76.137.110|8080|Japan|Osaka|ASAHI Net, Inc.|
|4|210.223.44.230|3128|South Korea|Seoul|Korea Telecom|
|5|103.102.12.134|1111|Indonesia|Rembang|FISNET|
|6|45.125.67.37|8443|Hong Kong|Fanling|Tele Asia Limited|
|7|103.171.183.255|1111|Indonesia|Ngamprah|PT Hayat Teknologi Informatika|
|8|47.77.216.82|1080|United States|Minkler|Alibaba.com LLC|
|9|160.19.18.42|8080|Indonesia|Semarang|PT Indo Telemedia Solusi|
|10|160.250.226.118|8090|Indonesia|Cirebon|PT Apci Network Solutions|
|11|223.25.110.185|8080|Indonesia|Purbalingga|SinergiNet|
|12|157.230.220.25|4857|United States|North Bergen|DigitalOcean, LLC|
|13|103.189.63.149|56457|Indonesia|Denpasar|PT Blip Integrator Provider|
|14|103.203.232.69|1080|Indonesia|Malang|PT Inti Data Guna|
|15|103.156.74.163|8080|Indonesia|Banjar Tengah|PT Trika Global Media|
|16|210.16.85.42|8080|India|Chennai|Sri Vari Network Private Limited|
|17|113.160.132.26|8080|Vietnam|Bắc Ninh|VietNam Post and Telecom Corporation|
|18|103.131.19.51|8080|Indonesia|Babat|PT Global Media Data Prima|
|19|157.230.220.25|4857|United States|North Bergen|DigitalOcean, LLC|
|20|117.236.124.166|3128|India|Gandhinagar|BSNL Internet|



## Contributing

Contributions are welcome, and they are greatly appreciated! Every
little bit helps, and credit will always be given.

